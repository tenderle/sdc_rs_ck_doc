...
knitr::opts_chunk$set(
  ...
  collapse = TRUE,
  comment = ">",
  cache=TRUE #if set to FALSE the code is run every time again when knitting the document
)
```

## Introduction

The targeted record swapping method is a pre-tabular Statistical Disclosure Control (SDC) technique. Records holding a high risk of identification are swapped (depending on the risk level) with a similar record in a different area. Possible swap partners are all records matching a given similarity profile. The goal of this package is to apply record swapping to statistical tablesThe goal of this package is to produce a microdata file from which coherently protected tables can be derived. This algorithm can be applied to micro data sets and is based on C++.
...
* `similar`: column indices of variables in data which should be considered when swapping records. Only records having identical values in the specified variables will be swapped. 
* `hierarchy`: column indices of variables in data which refer to the (geographic) hierarchy of the micro data set (for instance province > district > county > municipality)
...
Note: In R indexing usually starts with 1, but because this package was implemented in C++ indexing here begins with 0. This will however change if the function `recordSwap()` has fully matured and transitions from a testing to production-ready.
